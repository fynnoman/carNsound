...
export default function GaugeCluster() {
  return (
    <section className="relative py-24 md:py-32 px-4 md:px-8 bg-void-900 hairline-t hairline-b">
...
function GaugeCard({ g, delay }: { g: Gauge; delay: number }) {
  ...
  return (
    <motion.div
      ...
      className="panel-hard border border-white/10 p-5 md:p-6 relative overflow-hidden"
    >
      <div className="flex items-start justify-between mb-4">
        <span className="vin text-white/50">{g.label}</span>
        <span className="mono text-[9px] text-oxide-400">● OK</span>
      </div>
      ...
      <div className="relative flex items-center gap-3">
        ...
        <div className="flex flex-col leading-none">
          <motion.span className="d-hero text-white text-4xl md:text-5xl tabular">
            {display}
          </motion.span>
          <span className="mono text-[10px] text-white/50 mt-1 tracking-widest">
            /{g.cap} · {g.unit}
          </span>
        </div>
      </div>
